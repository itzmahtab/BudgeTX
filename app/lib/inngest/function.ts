...
async function generateFinancialInsights(stats: { totalIncome: number; totalExpenses: number; byCategory: Record<string, number> }, month: string) {
...
function isTransactionDue(transaction: { lastProcessed: Date | null; nextRecurringDate: Date | null }) {
  ...
  const nextDue = new Date(transaction.nextRecurringDate ?? today);
